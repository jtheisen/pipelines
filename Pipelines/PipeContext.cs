...
namespace Pipelines
{
    ...
    public class LivePipeline
    {
        ...
        async Task Run()
        {
            ...
            {
                if (workerTask.TaskFactory?.Scheduler is TaskScheduler taskScheduler)
                {
    ...
    public class PipeReportBufferPart : PipeReportPart
    {
        public PipeReportBufferPart(PipeReportBufferState state, Int64 content, Int64 size)
        {
            State = state;
            Content = content;
            Size = size;
        ...
        public PipeReportBufferState State { get; }
        public Int64 Content { get; }
    ...
    public class PipeReportWorker : PipeReportPart
    {
        public PipeReportWorker(String name, WorkerInputProgress progress, PipeReportWorkerState state)
        {
            Name = name;
            Progress = progress;
            State = state;
        ...
        public WorkerInputProgress Progress { get; }
        public PipeReportWorkerState State { get; }
    }

    public class PipeReport
    {
        public PipeReport(PipeReportPart[] parts)
        {
            Parts = parts;
        }

        public PipeReportPart[] Parts { get; }
    }
}
